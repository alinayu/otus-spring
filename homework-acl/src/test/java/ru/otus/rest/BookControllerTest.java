...
class BookControllerTest {
    ...
    @WithMockUser
    @Test
    void deleteByIdShouldCheckUserRole() throws Exception {
        this.mvc.perform(delete("/books/1"))
    ...
    @WithMockUser
    @Test
    void saveShouldCheckUserRole() throws Exception {
        Book book = new Book("test");
        ObjectMapper objectMapper = new ObjectMapper();
        this.mvc.perform(post("/books")
                .contentType(APPLICATION_JSON_UTF8)
                .content(objectMapper.writeValueAsString(book)))
                .andExpect(status().isForbidden());
        verify(bookService, times(0)).save(book);
    }
    ...
    @WithMockUser
    @Test
    void updateShouldCheckUserRole() throws Exception {
        Book book = new Book(1, "test");
        ObjectMapper objectMapper = new ObjectMapper();
        this.mvc.perform(put("/books/1")
                .contentType(APPLICATION_JSON_UTF8)
